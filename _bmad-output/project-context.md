...
- **Default corpora are committed seed data — a rule, not a permission** (owner decision 2026-08-01): `data/bible/kjv.json` is the default Bible seeder and `data/song-book/sdah.json` the default song-book seeder, both committed under `data/`. The rule this replaces said the KJV corpus *"may not be committed under `data/`"* — **reversed**, because it forbade exactly what Story 21.1 must do. **The current tree has neither yet** — the hymnal still sits at `data/hymns.json` (seeded by `upsertHymns` in `src/lib/db/index.ts`, an every-boot upsert) and `bible_verses` has no writer outside the git-ignored `.work/` export, so a fresh clone has no scripture at all. Epics 21–22 move and commit both. Unchanged, and not to be confused with the above: congregation PII, `data/uploads/`, `data/local/`, decks and local DBs still never enter the repository
