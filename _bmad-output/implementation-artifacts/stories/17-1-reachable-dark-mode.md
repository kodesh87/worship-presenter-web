...
The readiness assessment recorded *"Dark mode is unreachable … the entire dark palette is dead code"*, and `DESIGN.md` repeats it. **Verified against the code on 2026-07-29: that is wrong.** Two surfaces already pin the class themselves, and they are the two used while a service is running:
...
7. **Given** this story ships, **When** the change set is reviewed, **Then** `DESIGN.md` is updated in the same change set — the "dead code" claim corrected, the theme control documented, and the measurements from AC-6 recorded. `AGENTS.md` requires it: a UI component with a visual delta updates `DESIGN.md`.
...
- Defect source: `_bmad-output/planning-artifacts/implementation-readiness-report-2026-07-29.md` (open item 2 — corrected above)
- Visual authority: `_bmad-output/planning-artifacts/ux-designs/ux-bic-pptx-workflow-2026-07-10/DESIGN.md`
- Behavioural counterpart: same folder, `EXPERIENCE.md` (note: five UX findings are routed to `bmad-ux` separately; the "dead code" correction is one of them)
