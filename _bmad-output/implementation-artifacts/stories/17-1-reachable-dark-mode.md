...
- **All seven AC are satisfied; six were verified in the running application, and AC-5 could not be (see below).**
- `tests/theme-chrome.test.mjs` is new — 17 assertions across four groups: the projected tree
  carries no theme token and no theme-coloured edge and stays closed to its four known files
  (AC-4), the two presenter surfaces keep their own `dark` wrapper (AC-3), the provider is
  mounted as a client boundary under a Server-Component layout (AC-1/AC-2), and the control is
  a labelled, dependency-free, mount-guarded button (AC-1). Negative-tested: injecting
  `border-2` into `ArtifactSlide` fails the edge guard while the token guard stays green, which
  is the split the two guards exist for.
...
- **Two pre-existing defects observed and left alone.** (a) `src/app/ServicesList.tsx:98`
  ...
  is not in this change set. (b) `npm run lint` reports 32 problems (15 errors, 17 warnings),
  all pre-existing; the one error this story introduced was fixed before commit and none of the
  six changed files appears in the lint output.
...
- **AC-4 is now met, and the reason it was not is worth keeping.** The original verification was
...
- **Two chromatic text pairs were failing AA on the dark surface and neither was a `dark:`
  override.** They were `-600` shades chosen against white, in files that became dark-switchable
  underneath them: the preview badges (indigo **2.54:1**, below even the 3:1 large-text floor) and
  `LogoutButton` (**3.76:1**). Fixed from `PRESENTER_TONE_CLASS`, which had already solved this
  problem for the one surface that was always dark. A story that added a contrast AC should not
  have shipped with those in it.
