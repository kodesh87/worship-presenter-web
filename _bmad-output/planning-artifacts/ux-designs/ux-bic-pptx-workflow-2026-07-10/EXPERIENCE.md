...
| `/services/[id]/slideshow` | **Plan cannot be built** — a `buildSlidePlan` throw names the failure on the black projection canvas and offers the two recovery routes (run-sheet, Admin → Artifacts), rather than a blank screen. **Changed 2026-07-31**, from a `destructive`-bordered `Card`: this is a projected URL, and a token-painted error surface followed the operator's theme in front of the room. It now matches the projector's own failure branch, which has always used literal colours for that reason. *An "empty plan" state is deliberately absent:* `slide-plan.ts:253` pushes the `welcome` leaf unconditionally at the head of every plan, so zero slides cannot occur. |
...
**Items 6 and 7 are questions the architecture spine explicitly routes to this file.** It states in its own *Deferred* that both are UX concerns owned by `EXPERIENCE.md`, and until 2026-07-30 this list carried neither. Recording them here is what makes the handoff received rather than merely sent; both are undecided, and neither is a decision the spine will make. (They were items 5 and 6 until 2026-07-31, when the toast-channel item arrived above them.)
