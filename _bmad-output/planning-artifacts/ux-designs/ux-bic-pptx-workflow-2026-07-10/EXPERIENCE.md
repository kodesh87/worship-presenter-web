...
| Language settings (admin) | `/admin` | The four settings, and there is deliberately no fifth (FR-24): `default_data_locale`, `default_bible_translation`, `default_song_book`, `ui_locale`. The first three are the corpus axis, the fourth the interface axis; they move independently. Landed on by Flow 9 | **⚠ designed, not shipped.** *Owners: Story 21.3 (`default_data_locale`, `default_bible_translation`), Story 22.3 (`default_song_book`), Story 24.1 (`ui_locale`).* `src/lib/settings.ts` holds `RETENTION_KEY` and `SLIDE_TRANSITION_KEY` only |
| UI-locale switcher | `/admin` | Sets the language the operator interface is written in, and `<html lang>` with it. **It reaches no room-facing surface** — there is no `projection_locale`, by decision (PRD §4.12), so this is AD-24's closure read in the other direction. Landed on by Flow 9 | **⚠ designed, not shipped.** *Owner: Story 24.1.* `lang="en"` is hard-coded at `src/app/layout.tsx:31` and is the entirety of this product's internationalisation |
...
| `sonner` toasts | Transient confirmations only. Never the sole channel for an error that blocks work. **⚠ Specified, not shipped** — *Owner: Story 17.6.* `Toaster` is mounted nowhere and `toast(` is called nowhere in `src/` (verified 2026-07-31), so this pattern describes a channel that cannot currently fire. Open Item 4. |
...
- The dark palette **has now been measured**, on 2026-07-30 by Story 17.1 (AC-6), and its four text pairs pass: 18.97:1 `foreground` on `background`, 14.23:1 `primary-foreground` on `primary`, 7.66:1 and 5.86:1 for the two `muted-foreground` pairs ([`DESIGN.md`](./DESIGN.md) → *The same four pairs in the dark palette*). This bullet previously said the contrast had never been measured on any pair, which was true from 2026-07-19 until that run — and it mattered because the surfaces in question are the presenter and slide grid, the two an operator uses while a service is running. Two consequences worth carrying: the AA failure on `muted-foreground`/`muted` is **light-theme only**, and the amber affordances layered on the presenter are still unmeasured there ([`DESIGN.md`](./DESIGN.md) → *Open Item 4*).
...
### Flow 9 — the hub learns a second language *(⚠ designed, not shipped throughout)*

*Owners: Story 21.3, Story 22.3, Story 24.1.* Like Flow 8 it has no PRD user journey — FR-24 and FR-25 arrived on 2026-08-01 without one — and it is here because otherwise three sub-surfaces are landed on by nothing. **Every beat below is unbuilt**; the marker is stated once here rather than on each step.

1. A second Bible translation and an Indonesian song book arrive as committed corpus files and register themselves on boot (AD-26). **This is not a surface** — there is no install screen in scope, and inventing one is how a data capability grows a route nobody asked for.
2. Bimo opens **Settings**. Language sits in its own block, and the two axes are separated there because they are separated everywhere: **UI Locale** is the language he reads the hub in, **Data Locale** is the language of the corpora. He sets the interface to Indonesian; the page says so to a screen reader too, because `<html lang>` follows the setting rather than staying at the hard-coded `en`.
3. He sets the corpus default to Indonesian and picks the default translation. Nothing about the English corpora changes — they are still installed and still reachable.
4. **Climax:** Sabbath, Elen prepares a service that is Indonesian throughout except one English hymn the choir has rehearsed. The hymn picker opens on the Indonesian song book — and the English one is **visible from that same control**, not behind a settings change. She picks the hymn, and the next song's picker opens on Indonesian again. **She changed no setting to cross a language, and crossing it did not change the default for anyone else.** Had the default reached the query instead of the view, this service would have been impossible without an administrator, on a Sabbath morning, for one hymn.
5. Nothing Bimo or Elen did here reaches the congregation's screen. The slides read what an Admin composed on the Artifact Registry canvas — there is no `projection_locale` and no setting that projects (PRD §4.12), which is Epic 17's closure read in the other direction.
...
Behavioral items this file owns. Token and visual-identity gaps — `metadata` boilerplate, `muted-foreground` contrast, the undesigned hues, non-text contrast — live in [`DESIGN.md`](./DESIGN.md) → *Open Items* and are not restated here. **Each item names the story key that owns it**, or says why it has none.

*Dark-mode choice used to be routed there too. It is closed* ([`DESIGN.md`](./DESIGN.md) *→ Open Item 2), so the pointer is gone rather than aimed at a struck-through entry.*
...
4. **The toast channel has no trigger — `sonner` is installed, mounted nowhere, and called by nothing.** *Owner: Story 17.6.* Two rows of this file's own *Component Patterns* and *DESIGN.md*'s *Components* describe `sonner` toasts as a shipped pattern (*"Transient confirmations only. Never the sole channel for an error that blocks work"*). Verified against `src/` on 2026-07-31: `Toaster` is exported from `src/components/ui/sonner.tsx` and rendered in no layout or page, and `toast(` appears nowhere in `src/`. So the described channel cannot fire, and every confirmation in the product is inline instead — which is a legitimate design, just not the one two artifacts document.

   **Recorded here, in this shape, on the owner's decision (2026-07-31).** Story 17.1's AC-5 says toasts must follow the theme; that is structurally satisfied — `sonner.tsx` already calls `useTheme()` and now resolves against a real provider — and cannot be *observed*, because nothing triggers a toast. Mounting `<Toaster />` to make it observable would have added a UI surface no story asked for, so the story disclosed the gap instead. It then lived only in a `sprint-status.yaml` `last_updated` comment, and this list's own preamble says why that is not enough: an item with no key is how a finding becomes permanent. The decision this item owes an answer to is whether transient confirmation is a channel this product wants at all — if not, the two documentation rows go and `sonner` is uninstalled, which is a smaller change than building it.
