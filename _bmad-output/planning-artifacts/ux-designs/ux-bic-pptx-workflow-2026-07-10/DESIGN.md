...
Estimates derived from Oklab lightness (`Y ≈ L³`), valid because every token is achromatic. **Not tool-measured** — the two marked *verify* must be confirmed with a contrast checker before use in any accessibility claim (Open Item 1).

| Combination | Approx. ratio | WCAG |
| --- | --- | --- |
| `foreground` on `background` | ~19:1 | AAA |
| `primary-foreground` on `primary` | ~17:1 | AAA |
| `muted-foreground` on `background` | **~4.7:1** | AA normal text, narrowly — verify |
| `muted-foreground` on `muted` | **~4.4:1** | **Below AA (4.5:1)** — verify |

`muted-foreground` is shadcn's default and carries secondary text throughout — the one combination in this palette that is not comfortably safe, closest to the boundary exactly where it is used most.
...
1. **`muted-foreground` contrast is borderline and unverified.** ~4.7:1 on `background`, ~4.4:1 on `muted` — the second is below AA for normal text, on the token that carries all secondary text. Measure both; if confirmed, darkening one token fixes it globally.
2. **Dark mode is unreachable.** `globals.css` defines a complete `.dark` block (33 tokens) behind `@custom-variant dark (&:is(.dark *))`, but **no `ThemeProvider` is mounted anywhere** — `next-themes` is imported only by `src/components/ui/sonner.tsx` for toast theming. Nothing adds `.dark` to `<html>`, so the entire dark palette is dead code. The prior version of this file claimed "Dark/light via existing theme tokens"; that was false. Mount a provider (a story, and a real one for a tool used in a dim sanctuary) or delete the dead block — keeping both is the worst option.
3. **`metadata` is still create-next-app boilerplate.** `src/app/layout.tsx` exports `title: "Create Next App"`, `description: "Generated by create next app"`, so every browser tab and bookmark of the production hub reads *Create Next App*. One-line fix; wording is product-owned.
