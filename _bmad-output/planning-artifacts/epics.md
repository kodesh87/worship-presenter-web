...
>
> **The parser is the harder half and a corpus fix does not reach it:** `parseScriptureRef` caps a book name at **two words** and ASCII letters, so `Song of Solomon 2:1` is rejected just as `Song of Songs 2:1` is — the correct name is untypeable exactly as the wrong one was, and book 22 is the *only* book whose name runs past two words, which is why the cap has hidden this long. `Kisah Para Rasul 1:8` is what *generous input* runs into on the first Indonesian translation. All of it is in the spine's *Deferred* under this story; an architecture run does not patch production code or data.
